...
const fetchGlobalStats = async () => {
  try {
    ...
    // 1. Fetch the relevant columns: 'status' instead of 'is_approved'
    ...
    // 2. Fetch product count (this part was mostly fine)
    const { count: productCount } = await supabase
      .from("products")
      .select('*', { count: 'exact', head: true });

    // 3. Logic based on your 'status' column (pending, approved, etc.)
      ...
      <div className="max-w-[1600px] mx-auto space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">Dashboard Center</h1>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          />
          <DashboardCard
            label="Total Approved Businesses"
            value={stats.activeTrials}
            icon={<CheckCircle2 size={24} />}
            color="green"
          />
          <DashboardCard
            label="Total Rejected Businesses"
              ...
              {stats.pendingApprovals > 0 ? (
                ...
                >
                  ...
                  <button
                    ...
                    className="w-full sm:w-auto bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-orange-600 transition-all flex items-center justify-center gap-2"
                  >
 ...
 className="flex items-center gap-2 text-[#ff3d00] font-black text-[10px] uppercase tracking-widest hover:translate-x-2 transition-transform">
...
function DashboardCard({ label, value, icon, trend, warning, color }: any) {
  ...
  return (
    <motion.div
      ...
      className={`bg-white p-8 rounded-[2.5rem] border-2 transition-all ${warning ? 'border-orange-200' : 'border-white shadow-sm'
        }`}
    >
