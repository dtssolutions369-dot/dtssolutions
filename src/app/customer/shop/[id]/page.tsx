...
import {
  ...
  User, ChevronLeft // Added ChevronLeft
} from "lucide-react";
...
export default function BusinessProfilePage() {
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        if (error.code === "23505") alert("You have already reviewed this shop!");
        else throw error;
      } else {
        setUserComment("");
        fetchBusinessData(); 
  ...
  if (loading) return <div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin text-[#ff3d00]" size={40} /></div>;
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-20">
      ...
      <section className="relative bg-gradient-to-r from-[#ff3d00] to-orange-400 pt-28 pb-40 px-6 overflow-hidden">
        <button 
          onClick={() => router.back()}
          className="group flex items-center gap-2 bg-white/20 backdrop-blur-xl border border-white/30 p-2 pr-4 rounded-full text-white hover:bg-white hover:text-slate-900 transition-all shadow-xl"
        >
        ...
        </button>{/* Abstract background shape for flair */}
        <div className="absolute top-0 right-0 w-96 h-96 bg-white/5 rounded-full -mr-20 -mt-20 blur-3xl"></div>
        
        <div className="max-w-[1400px] mx-auto text-center space-y-4 relative z-10">
      ...
      <main className="max-w-[1400px] mx-auto px-6 -mt-24 relative z-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 flex flex-col items-center text-center space-y-3 hover:translate-y-[-5px] transition-transform">
            <div className="w-14 h-14 bg-orange-50 rounded-2xl flex items-center justify-center text-[#ff3d00] shadow-inner"><MapPin size={24} /></div>
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Full Address</h4>
            <p className="text-slate-900 font-bold text-base leading-snug">{business?.address}</p>
            <p className="text-[#ff3d00] text-xs font-black">PIN: {business?.pincode}</p>
          </div>
          ...
          <DetailCard 
            ...
            subValue={`Owner: ${business?.owner_name}`}
            isAction
          />
          ...
          <DetailCard 
            ...
            subValue={`Member Since ${memberSince}`} 
          />
        </div>
                            ...
                            <div className="flex justify-between mb-3">
                                <div className="flex flex-col gap-1">
                                    <div className="flex gap-1 text-orange-400">
                                        {[...Array(5)].map((_, i) => <Star key={i} size={12} fill={i < rev.rating ? "currentColor" : "none"} className={i >= rev.rating ? "text-slate-200" : ""} />)}
                                    </div>
...
function ProductCard({ product }: any) {
    return (
      <Link href={`/customer/product/${product.id}`} className="group bg-white rounded-[2.5rem] p-4 border border-slate-100 hover:border-orange-100 hover:shadow-2xl hover:shadow-orange-200/50 transition-all flex flex-col h-full">
        <div className="aspect-square rounded-[2rem] overflow-hidden mb-4 bg-slate-100 relative">
          <img src={product.images?.[0]} className="w-full h-full object-cover group-hover:scale-110 transition-all duration-700" alt="" />
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors"></div>
        </div>
        <div className="px-2">
          <h3 className="font-bold text-slate-900 text-sm truncate group-hover:text-[#ff3d00] transition-colors">{product.name}</h3>
          <div className="flex items-center justify-between mt-2">
              <span className="text-xl font-black text-slate-900">₹{product.price?.toLocaleString()}</span>
              <div className="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-[#ff3d00] group-hover:text-white transition-all transform group-hover:translate-x-1">
                  <ArrowRight size={14} />
              </div>
          </div>
        </div>
